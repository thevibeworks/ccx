...
func renderInsightPage(result *insight.Summary) string {
	var b strings.Builder
	b.WriteString(pageHeader("Insight - ccx", "light"))
	b.WriteString(renderTopNav("", ""))
	b.WriteString(`<div class="layout">`)
	b.WriteString(renderSidebar("insight"))
	b.WriteString(`<main class="main-content insight-page">`)

	b.WriteString(`<div class="insight-hero">`)
	b.WriteString(`<div>`)
	b.WriteString(`<span class="page-badge badge-conversation">I</span>`)
	b.WriteString(`<h1>Insight</h1>`)
	if result != nil {
		b.WriteString(fmt.Sprintf(`<p>%s · %s · generated %s</p>`,
			html.EscapeString(result.Scope.Label),
			html.EscapeString(result.Scope.TimeZone),
			html.EscapeString(result.GeneratedAt.Format("2006-01-02 15:04"))))
	}
	b.WriteString(`</div>`)
	b.WriteString(`<form class="insight-controls" method="get" action="/insight">`)
	b.WriteString(`<select name="scope" class="sort-select">`)
	for _, scope := range insight.ScopeAliases() {
		selected := ""
		if result != nil && result.Scope.Name == string(scope) {
			selected = " selected"
		}
		b.WriteString(fmt.Sprintf(`<option value="%s"%s>%s</option>`, scope, selected, insightScopeLabel(scope)))
	}
	b.WriteString(`</select>`)
	tz := "local"
	if result != nil && result.Scope.TimeZone != "" {
		tz = result.Scope.TimeZone
	}
	b.WriteString(fmt.Sprintf(`<input name="tz" class="search-input insight-tz" value="%s" placeholder="Timezone">`, html.EscapeString(tz)))
	provider := ""
	project := ""
	if result != nil {
		provider = result.Scope.Provider
		project = result.Scope.Project
	}
	b.WriteString(`<select name="provider" class="sort-select" title="Provider">`)
	b.WriteString(fmt.Sprintf(`<option value=""%s>All agents</option>`, selected(provider, "")))
	b.WriteString(fmt.Sprintf(`<option value="cc"%s>Claude Code</option>`, selected(provider, "claude-code")))
	b.WriteString(fmt.Sprintf(`<option value="cx"%s>Codex</option>`, selected(provider, "codex")))
	b.WriteString(`</select>`)
	b.WriteString(fmt.Sprintf(`<input name="project" class="search-input insight-project" value="%s" placeholder="Project or path">`, html.EscapeString(project)))
	b.WriteString(`<button class="icon-btn insight-run" type="submit">Run</button>`)
	b.WriteString(`</form>`)
	b.WriteString(`</div>`)

	if result == nil || result.Metrics.Sessions == 0 {
		b.WriteString(`<div class="empty-state">No sessions in this scope.</div>`)
		b.WriteString(`</main></div>`)
		b.WriteString(renderFooter())
		b.WriteString(indexJS())
		b.WriteString(insightPageCSS())
		b.WriteString(pageFooter())
		return b.String()
	}

	b.WriteString(`<section class="insight-metrics">`)
	writeInsightMetric(&b, "Sessions", fmt.Sprintf("%d", result.Metrics.Sessions), "agent work blocks")
	writeInsightMetric(&b, "Projects", fmt.Sprintf("%d", result.Metrics.Projects), "active workspaces")
	writeInsightMetric(&b, "Tools", fmt.Sprintf("%d", result.Metrics.ToolCalls), "execution pressure")
	writeInsightMetric(&b, "Tokens", formatTokens(result.Metrics.TotalTokens), "context spent")
	writeInsightMetric(&b, "Cost", formatCost(result.Metrics.CostUSD), "estimated spend")
	b.WriteString(`</section>`)

	b.WriteString(`<section class="insight-grid">`)
	b.WriteString(renderInsightSessionPanel("Currently being worked on", result.Current))
	b.WriteString(renderInsightSignalPanel("Needs closure", result.OpenLoops))
	b.WriteString(renderInsightSessionPanel("Completed", result.Completed))
	b.WriteString(renderInsightSignalPanel("Emerging signals", result.Patterns))
	b.WriteString(`</section>`)

	b.WriteString(renderInsightProjectTable(result.Projects))
	b.WriteString(`</main></div>`)
	b.WriteString(renderFooter())
	b.WriteString(indexJS())
	b.WriteString(insightPageCSS())
...
func writeInsightMetric(b *strings.Builder, label, value, hint string) {
	b.WriteString(`<article class="insight-metric">`)
	b.WriteString(fmt.Sprintf(`<div class="insight-metric-value">%s</div>`, html.EscapeString(value)))
	b.WriteString(fmt.Sprintf(`<div class="insight-metric-label">%s</div>`, html.EscapeString(label)))
	b.WriteString(fmt.Sprintf(`<div class="insight-metric-hint">%s</div>`, html.EscapeString(hint)))
	b.WriteString(`</article>`)
}

func renderInsightSessionPanel(title string, sessions []insight.SessionRef) string {
	var b strings.Builder
	b.WriteString(`<section class="insight-panel"><h2>`)
	b.WriteString(html.EscapeString(title))
	b.WriteString(`</h2>`)
	if len(sessions) == 0 {
		b.WriteString(`<p class="insight-muted">No signal.</p></section>`)
		return b.String()
	}
	for _, s := range sessions {
		b.WriteString(`<article class="insight-item">`)
		b.WriteString(`<div class="insight-item-top">`)
		b.WriteString(providerBadgeHTML(s.Provider))
		b.WriteString(fmt.Sprintf(`<span>%s</span>`, html.EscapeString(s.End.Format("Jan 02 15:04"))))
		b.WriteString(fmt.Sprintf(`<code>%s</code>`, html.EscapeString(truncate(s.ID, 8))))
		b.WriteString(`</div>`)
		b.WriteString(fmt.Sprintf(`<h3>%s</h3>`, html.EscapeString(s.Project)))
		b.WriteString(fmt.Sprintf(`<p>%s</p>`, html.EscapeString(truncate(s.Summary, 180))))
		b.WriteString(`<div class="insight-item-stats">`)
		b.WriteString(fmt.Sprintf(`<span>%d messages</span><span>%d tools</span><span>%s tokens</span>`,
			s.Messages, s.ToolCalls, formatTokens(s.Tokens)))
...
func renderInsightSignalPanel(title string, signals []insight.Signal) string {
	var b strings.Builder
	b.WriteString(`<section class="insight-panel"><h2>`)
	b.WriteString(html.EscapeString(title))
	b.WriteString(`</h2>`)
	...
	for _, signal := range signals {
		count := ""
		if signal.Count > 0 {
			count = fmt.Sprintf(` <span class="insight-count">%d</span>`, signal.Count)
		}
		b.WriteString(`<article class="insight-signal">`)
		b.WriteString(fmt.Sprintf(`<div class="insight-signal-label">%s%s</div>`, html.EscapeString(signal.Label), count))
		b.WriteString(fmt.Sprintf(`<p>%s</p>`, html.EscapeString(signal.Summary)))
		b.WriteString(fmt.Sprintf(`<span class="insight-confidence">%s confidence</span>`, html.EscapeString(signal.Confidence)))
		b.WriteString(`</article>`)
	}
	b.WriteString(`</section>`)
	return b.String()
...
func renderInsightProjectTable(projects []insight.ProjectRow) string {
	if len(projects) == 0 {
		return ""
	}
	var b strings.Builder
	b.WriteString(`<section class="insight-table-section"><h2>Project focus</h2>`)
	b.WriteString(`<table class="insight-table"><thead><tr><th>Project</th><th>Sessions</th><th>Messages</th><th>Tools</th><th>Tokens</th><th>Cost</th></tr></thead><tbody>`)
	for _, project := range projects {
		b.WriteString(`<tr>`)
		b.WriteString(fmt.Sprintf(`<td>%s</td><td>%d</td><td>%d</td><td>%d</td><td>%s</td><td>%s</td>`,
...
func insightScopeLabel(scope insight.Scope) string {
	...
	case insight.ScopeMonth:
		return "This month"
	case insight.ScopeQuarter:
		return "This quarter"
	case insight.ScopeYear:
		return "This year"
	default:
		return "Today"
	}
...
func insightPageCSS() string {
	return `<style>
.insight-page { max-width: 1180px; }
.insight-hero { display:flex; justify-content:space-between; align-items:flex-start; gap:20px; margin-bottom:22px; }
.insight-hero h1 { font-size:32px; line-height:1.1; margin:8px 0 4px; }
.insight-hero p { color:var(--text-muted); font-size:14px; }
.insight-controls { display:flex; gap:8px; align-items:center; }
.insight-tz { width:210px; font-size:13px; padding:7px 10px; }
.insight-project { width:180px; font-size:13px; padding:7px 10px; }
.insight-run { height:34px; }
.insight-metrics { display:grid; grid-template-columns:repeat(5, minmax(0,1fr)); gap:10px; margin-bottom:18px; }
.insight-metric { border:1px solid var(--border); background:var(--bg-secondary); border-radius:6px; padding:14px; min-height:96px; }
.insight-metric-value { font-size:25px; font-weight:750; letter-spacing:0; }
.insight-metric-label { font-size:12px; color:var(--text); margin-top:4px; }
.insight-metric-hint { font-size:11px; color:var(--text-muted); margin-top:6px; }
.insight-grid { display:grid; grid-template-columns:repeat(2, minmax(0,1fr)); gap:14px; }
.insight-panel, .insight-table-section { border:1px solid var(--border); border-radius:6px; background:var(--bg-secondary); padding:16px; }
.insight-panel h2, .insight-table-section h2 { font-size:14px; text-transform:uppercase; color:var(--text-muted); letter-spacing:0; margin-bottom:12px; }
.insight-item, .insight-signal { border-top:1px solid var(--border); padding:12px 0; }
.insight-item:first-of-type, .insight-signal:first-of-type { border-top:0; padding-top:0; }
.insight-item-top { display:flex; align-items:center; gap:8px; color:var(--text-muted); font-size:12px; margin-bottom:6px; }
.insight-item h3 { font-size:15px; margin-bottom:4px; }
.insight-item p, .insight-signal p { font-size:13px; color:var(--text); line-height:1.45; }
.insight-item-stats { display:flex; gap:10px; color:var(--text-muted); font-size:11px; margin-top:8px; }
.insight-signal-label { font-weight:650; font-size:13px; margin-bottom:4px; }
.insight-count { display:inline-flex; align-items:center; justify-content:center; min-width:20px; height:20px; border-radius:10px; background:var(--bg-tertiary); font-size:11px; margin-left:4px; }
.insight-confidence { color:var(--text-muted); font-size:11px; }
.insight-muted { color:var(--text-muted); font-size:13px; }
.insight-table-section { margin-top:14px; }
.insight-table { width:100%; border-collapse:collapse; font-size:13px; }
.insight-table th, .insight-table td { padding:9px 8px; border-top:1px solid var(--border); text-align:left; }
.insight-table th { color:var(--text-muted); font-weight:600; border-top:0; }
@media (max-width:900px) {
  .insight-hero { flex-direction:column; }
